...
Тест № 1.1 Проверка рисования линии(позитивный)
 ...
 - Входные данные: Смоделировать событие <Motion> с координатами (10, 10) и (20, 20).
 - Ожидаемый результат: На холсте make_canvas должна быть нарисована линия от (10, 10) до (20, 20) с заданными параметрами (цвет, ширина).
...
Тест №2.1: Проверка отсутствия предыдущих координат(негативный)
   ...
   - Входные данные: Смоделировать событие <Motion> с координатами (20, 20), при этом self.old_x и self.old_y должны быть None.
...
Тест №2.2: Проверка недопустимых параметров(негативный)
   ...
   - Входные данные: Смоделировать событие <Motion> с координатами (10, 10), при этом self.fill_color_line, self.width_maintainer или другие параметры должны иметь недопустимые значения.
...
Тест №1.1: Проверка рисования окружности(позитивный)
   ...
   - Входные данные: Смоделировать событие <Motion> с координатами (100, 100) и (200, 200).
   - Ожидаемый результат: Координаты объекта на холсте должны соответствовать ожидаемым
...
Тест №1.2: Проверка добавления окружности в список temp(позитивный)
   ...
   - Входные данные: Смоделировать событие <Motion> с координатами (50, 50) и (150, 150).
...
Тест 2.1 Некорректные координаты(негативный)
   ...
   - Входные данные: Некорректные координаты события (-100, 1000) 
   - Ожидаемый результат: Функция должна обработать некорректные координаты без вызова исключений или ошибок.
   - Описание процесса: Вызвать функцию circle_ranging с некорректными координатами события (-100, 1000). Проверить, что функция не вызвала исключений или ошибок.

Тест №2.2 отсутствие_первого_клика(негативный)
   - Цель: проверка корректной работы функции при отсутствии первого клика.
   - Входные данные: Координаты второго клика (100, 200).
